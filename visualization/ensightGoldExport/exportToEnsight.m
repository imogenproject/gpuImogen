function exportToEnsight(frame)
...
makeEnsightScalarFile([basename '.mass'], 0, 2, frame.mass, 'mass');

fprintf(CASE, 'scalar per node: 	energy %s.ener.**\n', basename);
makeEnsightScalarFile([basename '.ener'], 0, 2, frame.ener, 'energy');

if ~isempty(frame.grav)
    fprintf(CASE, 'scalar per node:             grav_potential %s.grav.**\n', basename);
    makeEnsightScalarFile([basename '.grav'], 0, 2, frame.grav, 'gravity');
end

fprintf(CASE, 'vector per node:		momentum %s.mom.**\n', basename);
makeEnsightVectorFile([basename '.mom'], 0, 2, frame.momX, frame.momY, frame.momZ, 'momentum');

if ~isempty(frame.magX)
    fprintf(CASE, 'vector per node:		magnet %s.mag.**\n', basename);
    makeEnsightVectorFile([basename '.mag'], 0, 2, frame.magX, frame.magY, frame.magZ, 'magnetic_field');
end
