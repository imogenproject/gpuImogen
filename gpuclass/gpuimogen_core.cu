...
int readImogenICs(GridFluid *g, MGArray *holder, GeometryParams *geo, char *h5dfilebase, int frameNo)
{
	double *q = (double *)malloc((unsigned long)(sizeof(double)*geo->localRez[0] * geo->localRez[1] * geo->localRez[2]));
	...
	// Read rho
	if(IHR.readDoubleArray("/fluid1/mass", &q) < 0) {
		PRINT_FAULT_HEADER; printf("Unsuccessful reading /fluid1/ener from %s", hfile); PRINT_FAULT_FOOTER;
	...
	const char *flufields[] = {"/fluid1/ener", "/fluid1/momX", "/fluid1/momY", "/fluid1/momZ"};
	for(i = 0; i < 4; i++) {
		j = i + 1;

		// read from file to memory
		if(IHR.readDoubleArray(flufields[i], &q) < 0) {
			PRINT_FAULT_HEADER; printf("Unsuccessful reading %s from %s", flufields[i], hfile); PRINT_FAULT_FOOTER;
			free(q);
			return ERROR_FILE_IO;
		}
		// upload to GPU
...
int readGravityPotential(ImogenH5IO *conf, GravityData *gravdat, MGArray *fluidReference)
{
	...
	if(haveg) {
		...
		if(myrank == 0) {
			std::cout << "Gravitational potential enabled; Reading from '" << potfile << "'\n" << endl;
		}
		...
		printf("pot size: %i %i %i\n", gravdims[0], gravdims[1], gravdims[2]);
