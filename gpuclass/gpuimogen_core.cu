...
int main(int argc, char **argv)
{
	...
	int cf = lookForArgument(argc, argv, "--initfile");
	if(cf > 0) {
		if(cf >= (argc-1)) {
			// error - there must be an argument after --initfile
			MPI_Finalize();
			if(myrank > 0) return -1;
			std::cout << "Error:\n\t--initfile is the last argument.\n\tA file must be named after --initfile,\n\timogenCore --initfile foo.h5\n" << std::endl;
			return -1;
		} else {
			configFilename = argv[cf+1];
		}
	} else { // No --initfile argument!
		MPI_Finalize();
		if(myrank > 0) return -0;
