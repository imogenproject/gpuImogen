...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  double exponent  = *mxGetPr(prhs[6+3*mode]);
  double strength  = *mxGetPr(prhs[7+3*mode]);
  ...
  switch(mode + 2*nlhs) {
    case 0: {
      ...
      break; }
    case 1: { double **B = getGPUSourcePointers(prhs, &amd, 5, 7);
    ...
    case 2: {
      ...
      break; }
    case 3: { double **B = getGPUSourcePointers(prhs, &amd, 5, 7);
