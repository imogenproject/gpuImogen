...
if B ~= 0; error('Scalar GPU ul/dl: Returned data failed to be exactly equal to original: Failure.');
    else; disp('	UL/DL successful!'); end

% SINGLE DEVICE DEEP COPY
Cd = GPU_clone(Ad);
B = max(max(abs(GPU_download(Cd) - Ad.array)));
if B ~= 0; error('GPU_clone data was not identical to original data after download: Failure.');
...
if B ~= 0; error('Multi-device ul/dl: Returned data failed to be exactly equal to original: Failure.');
    else; disp('	UL/DL successful!'); end
...
B = max(max(abs(GPU_download(Cd) - Ad.array)));
if B ~= 0; error('GPU_copy does not work.');
