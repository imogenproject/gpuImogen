...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
 ...
 m = GPU_ctrl('memory'): m[i,:] = [free, total] on device i.\n");
	...
	if(strcmp(c,"destroyStreams") == 0) {
		if(nrhs < 3) { printf("Call is: GPU_ctrl('destroyStreams', [device list], streams"); return; }
		cudaStream_t *streams = (cudaStream_t *)mxGetData(prhs[0]);
		int numd;
		double *d = mxGetPr(prhs[1]);
		numd = (int)mxGetNumberOfElements(prhs[1]);

		uint64_t *s = (uint64_t *)mxGetData(prhs[2]);
		int nums = (int)mxGetNumberOfElements(prhs[2]);
		...
		for(i = 0; i < numd; i++) {
			cudaSetDevice((int)d[i]);
			for(j = 0; j < nspd; j++) {
				cudaStreamDestroy((cudaStream_t)s[numd*j + i]);
			}
		}
	}
