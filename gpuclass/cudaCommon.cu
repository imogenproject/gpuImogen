...
/* cudaCommon.cu responds to the following -D options:
 * -DALLOCFREE_DEBUG
...
void calcPartitionExtent(MGArray *m, int P, int *sub)
{
	...
	if(P >= m->nGPUs) {
		...
		printf((const char *)"Error: Requested partition %i but only %i GPUs in use.\nReturning garbage\n", P, m->nGPUs);
		for(i = 0; i < 6; i++) { sub[i] = -1; }
	}
...
/* Teeeechnically these 2 are Matlab interface functions, but they're also potentially
 * useful as general purpose and they don't actually call the Matlab API so they can stay
 */
 ...
 *
 * If *ret is NULL, allocates N MGArrays on the heap & writes *ret.
 * If *ret is not NULL, assumes *ret refers to existing storage for N MGArrays.
 */
int MGA_allocArrays(MGArray **ret, int N, MGArray *skeleton)
{
	MGArray *m;
	// Do some preliminaries,
	if(*ret == NULL) { // allocate the MGArrays on heap
		m = (MGArray *)malloc(N*sizeof(MGArray));
		*ret = m;
	} else {
		m = *ret;
	}
 ...
 *
 * don't use this... I'm like 90% sure it can be trashcanned
 */
...
/* I'm 98% sure these can be trashcanned in favor of the type constructors */
...
/* Compute the # of elements on a whole face. If the face is the partition direction,
 * only one to calculate. Otherwise, sum of # elements in all facing partitions */
int MGA_wholeFaceHaloNumel(MGArray *a, int direction, int h)
{
if(a == NULL) {
	PRINT_SIMPLE_FAULT("Error: MGA_wholeFaceHaloNumel(NULL, ...) detected. Returning zero.\n");
	return 0;
}
 ...
 *     g->partNumel[partitionFrom] or g->numel depending on above.
 * If p[0] != NULL, p[0] must point to sufficient storage or we will segfault. */
// FIXME: this entire mess should just make a few calls to cudaMemcpy2D/3D
...
void MGA_debugPrintAboutArray(MGArray *x)
{
	...
	printf("   [");
	for(j = 0; j < 6; j++) { printf("%i ", x->boundaryConditions.mode[j]); }
	printf("]\n");
	printf("   externalData = %lx\n", (unsigned long)x->boundaryConditions.externalData);
	printf("   extIndex     = %i\n", x->boundaryConditions.extIndex);
	printf("==========\n");
