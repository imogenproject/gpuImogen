...
%___________________________________________________________________________________________________ calculateInitialConditions
        function [mass, mom, ener, mag, statics, potentialField, selfGravity] = calculateInitialConditions(obj)
            ...
            if obj.perturb 
	        if ceil(max(obj.grid*.0058))<3
                halfPlus =      ceil(min(half+3,obj.grid));
                halfMinus =     ceil(max(half-3,1));
                else
                        halfPlus    = ceil(min(half+obj.grid*.0058,obj.grid)); % min(half+3,obj.grid);       ceil(.0058*Obj.Grid(1))
                        halfMinus   = ceil(max(half-obj.grid*.0058,1));         % max(half-3,1);
                end
