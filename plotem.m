...
if exist('run','var')
    ...
    thingB = run.fluid(fn).mom(p2).array./run.fluid(fn).mass.array;
    ...
    if numel(find(size(run.fluid(fn).mass.array) > 1)) > 1 % 2d
        ...
        if numel(find(size(run.fluid(fn).mass.array) > 1)) > 2 % 3d
            ...
            subplot(2,2,3); imagesc(squeeze(logger(run.fluid(fn).mass.array(:,:,z0)))); title('log10[mass]'); colorbar;
            ...
            subplot(2,2,3); imagesc(squeeze(logger(run.fluid(fn).mass.array))); title('log10[mass]'); colorbar;
        ...
        for f = 1:numel(run.fluid)
            ...
            subplot(2,2,3); plot(logger(S.mass.array)); title('log10[mass]');
            subplot(2,2,1); plot(S.mom(1).array./S.mass.array); title('v(1)');
            subplot(2,2,2); plot(S.mom(p2).array./S.mass.array); title('v(2)');
    ...
    if numel(find(size(mass.array) > 1)) > 1 % 2d
        subplot(2,2,3); imagesc(logger(mass.array)); title('log10 mass'); colorbar;
        ...
        subplot(2,2,4); imagesc(logger(eint)); title('log10[Einternal]'); colorbar;
    else
        subplot(2,2,3); plot(logger(mass.array)); title('mass');
        subplot(2,2,1); plot(mom(1).array); title('mom(1)');
        subplot(2,2,2); plot(mom(p2).array); title('mom(2)');
